 ...
 *
 *To post data using an object
 *		var data = {C_PICKINDICATOR:1, SITEID:TAR}
 *		SOA004Client.postobj( "ID", data );
 *
 *
 *Methods can be run async as well using the getasynbc and postasync, adding a callback method to the methods parameters. The parsed json will be passed to the callback as it's only argument
 */
...
SOA004Client = {
	...
	get: function( filter, id, fields ) {
		...
		if( this.system=="" || this.dataobject=="" ) return null;
	...
	postobjasync: function( id, obj, callback ) {
		var data = [];
		for( var prop in obj ) {
			if( obj.hasOwnProperty( prop ) ) {
				var o = {};
