function SaveItem() {
  ...
  var rownum = document.forms.TimeSheet.rownum.value;
  var wonum = document.forms.TimeSheet.wonum.value;
	var labor = document.forms.TimeSheet.labor.value;
  var timedate = document.forms.TimeSheet.timedat.value;
  var timetype = document.forms.TimeSheet.timetype.value;
  var acthours = document.forms.TimeSheet.acthours.value;

	SessionStorage.setItem(rownum, wonum, labor, timedate, timetype, acthours);
	doShowAll();
...
function doShowAll() {
	if (CheckBrowser()) {
		...
		alert('Din webläsare stödjer inte denna funktion');
	}
}
